...
public final class DuneSkillIconRenderer {
    private static final ResourceLocation DUNE_ICON =
            ResourceLocation.fromNamespaceAndPath(TrimMOD.MODID, "textures/gui/dune.png");
    private static final int ICON_SIZE = 22;

    private DuneSkillIconRenderer() {
    }
    ...
    static void renderIcon(GuiGraphics guiGraphics, Player player) {
        ...
        RenderSystem.setShaderTexture(0, DUNE_ICON);
        RenderSystem.enableBlend();
        guiGraphics.blit(DUNE_ICON, x, y, 0, 0.0f, ICON_SIZE, ICON_SIZE, ICON_SIZE, ICON_SIZE);
    ...
    static float getCooldownPercent(Player player) {
        return TrimSkillCooldowns.getCooldownPercent(player, TrimPatterns.DUNE);
    }

    static boolean isFullDuneSet(Player player) {
        for (ItemStack armor : player.getArmorSlots()) {
            if (armor.isEmpty()) return false;
            var trim = ArmorTrim.getTrim(player.level().registryAccess(), armor);
            if (trim.isEmpty() || !trim.get().pattern().is(TrimPatterns.DUNE)) {
                return false;
            }
        }
        return true;
