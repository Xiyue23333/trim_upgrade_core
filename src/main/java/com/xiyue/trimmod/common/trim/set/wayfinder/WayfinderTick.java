...
import com.xiyue.trimmod.network.PacketSyncWayfinderEnergy;
import net.minecraft.server.level.ServerPlayer;

public final class WayfinderTick {
    private static final String ENERGY_TAG = PersistentDataKeys.WAYFINDER_ENERGY;
    private static final String TICK_TAG = PersistentDataKeys.WAYFINDER_REGEN_TICKS;
    ...
    public void tickEnergy(ServerPlayer player, boolean hasFullSet) {
        if (hasFullSet && !player.getPersistentData().contains(ENERGY_TAG)) {
            ...
            ModMessages.sendToPlayer(new PacketSyncWayfinderEnergy(100), player);
        }

        int energy = player.getPersistentData().getInt(ENERGY_TAG);
        if (hasFullSet) {
            int fullRechargeTicks = Config.cooldownTicks(Config.cooldownWayfinderSeconds);
            if (fullRechargeTicks <= 0) {
                ...
                if (newEnergy != energy) {
                    ...
                    ModMessages.sendToPlayer(new PacketSyncWayfinderEnergy(newEnergy), player);
                }
                if (newEnergy >= 100) {
                    ticks = 0;
                }
        ...
        if (energy > 0 || player.getPersistentData().getInt(TICK_TAG) > 0) {
            ...
            player.getPersistentData().putInt(TICK_TAG, 0);
            ModMessages.sendToPlayer(new PacketSyncWayfinderEnergy(0), player);
        }
    }
}
